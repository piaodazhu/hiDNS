...
u_int64_t g_avr_delay_us;
u_int64_t g_min_delay_us;
u_int64_t g_max_delay_us;
...
{
    FILE    *file;
    char     buf[1024], domain[255], qtype[10], mincn[10], maxcn[10];
    int      len = 0, qtype_n;
    data_t  *d;
    ...
    while(fgets(buf, 1024, file) != 0) {
        ...
        if (sscanf(buf, "%s %s %s %s", domain, qtype, mincn, maxcn) == EOF) {
...
int dns_perf_generate_query(query_t *q)
{
    ...
    ins_qry_buf *qbuf = (ins_qry_buf*)q->send_buf;
    len = ins_init_query_buf(qbuf, qbuf->buf + INS_UDPMAXSIZE, q->data->domain, strlen(q->data->domain));
	qbuf->header.qtype = INS_T_A;
	qbuf->header.maxcn = q->data->maxcn;
	qbuf->header.mincn = q->data->mincn;
...
int dns_perf_query_process_response(query_t *q, unsigned short id)
{
    ...
    timeval_t now, diff;
    u_int64_t delay_us = 0;
    gettimeofday(&now, NULL);
    diff = dns_perf_timer_sub(now, q->ts);
    delay_us = diff.tv_sec * 1000000 + diff.tv_usec;
    g_avr_delay_us += delay_us;
    if (delay_us > g_max_delay_us)
        g_max_delay_us = delay_us;
...
static void dns_perf_statistic()
{
    ...
    avr_delay = g_avr_delay_us * 1.0 / g_recv_number;
    printf("[Result]Average Latency(us):\t%.3f\n", avr_delay);
    printf("[Result]Maximum Latency(us):\t%lu\n", g_max_delay_us);
    printf("[Result]Minimum Latency(us):\t%lu\n", g_min_delay_us);
}
